...
document.querySelector('#save_out').addEventListener('click', A_save_difference,false) 
...
document.querySelector('#dismiss1').addEventListener("click",A_dismiss,false)//取消存檔
document.querySelector('#dismiss2').addEventListener("click",A_dismiss,false)//取消存檔
...
function PPP_notes_DOM(i,add,data){
      ...
      + PPPP_noteLabel_get__allData(partLabel)
      +"</div></div>";
    return add
}
...
//印出標籤
function PPPP_noteLabel_get__allData(item){
  var len =item.length
  var str =""
  for (var i = 0; i<len; i++){
    // console.log(item[i])
    str +=`<span class="badge badge-pill badge-secondary mr-1"> ${item[i]} </span>`
  }
  return str
}
...
function A_save_difference(){
  ...
  if(name == ""){ 
    ...
    task = allData[pos].label
    P_addNoteClass_print__task(task)
    let love_value = allData[pos].star
    // console.log(love_value)
    if(love_value == true){
      document.querySelector("#addMyLove").checked=true
    }else{
      document.querySelector("#addMyLove").checked=false
    }
...
function A_addNoteClass_check__task(){
  ...
  if(str ==""){
    ...
    if(task.indexOf(str)>-1) {
      ...
      // console.log("沒有新增重複")
      task.push(str)//資料儲存
      P_addNoteClass_print__task(task) //介面更新-新增回饋+更新使用者本次所選取資料(task陣列)
    }
  }
}
...
function P_addNoteClass_print__task(oldTask){
  ...
  if(oldTask.length==0){
    ...
    for(let i=0; i<oldTask.length; i++){
      let ele = document.querySelector("#noteClass");
      let newSpan = document.createElement("span")
      let newButton = document.createElement("button")
      newSpan.innerHTML = oldTask[i];
...
//刪除功能
function P_Node_delete__allData(name){
  console.log("執行刪除")
  pos = allData.map(function(e) { return e.noteName; }).indexOf(name);
  // console.log(pos)
